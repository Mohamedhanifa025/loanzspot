...
@section('content')
            ...
            <div class="header-body">
                <div class="row align-items-center pt-2 pb-7">
                    <div class="col-md-6 animated fadeInUp">
                        <h1 class="h1 text-white d-inline-block mb-2"><i class="fa fa-building mr-2"></i> {{ trans('global.channel.title') }}</h1>
                    </div>
                    <div class="col-md-6 animated fadeInUp text-md-right">
                        @can('channel_create')
                            <a class="btn btn-sm btn-white mt-2" href="{{ route("admin.channels.create") }}">
    ...
    <div class="container-fluid mt--9">
                ...
                <div class="card animated fadeInUp">
                    ...
                    <div class="table-responsive">
                        <table class="datatable table align-items-center table-flush">
                            <thead>
                            <tr>
                                <th scope="col" data-orderable="false">{{ trans('global.channel.fields.channel_id') }}</th>
                                <th scope="col" data-orderable="false">{{ trans('global.channel.fields.name') }}</th>
                                <th scope="col" data-orderable="false">{{ trans('global.channel.fields.joining_date') }}</th>
                                <th scope="col" data-orderable="false">{{ trans('global.channel.fields.mobile_number') }}</th>
                            ...
                            @foreach($channels as $key => $channel)
                                <tr data-entry-id="{{ $channel->id }}">
                                    ...
                                    <td>
                                        {{ $channel->joining_date ?? '' }}
                                    </td>
                                    <td>
                                        {{ $channel->mobile_number ?? '' }}
                                    </td>
                                    <td class="font-weight-bold text-center">
                                        <span class="py-1 text-uppercase px-2 rounded small {{ $channel->status == 1 ? 'bg-success':'bg-danger' }} text-white">{{ $channel->status_text ?? '' }}</span>
                                    </td>
                                    <td class="actions text-center">
                                        ...
                                        @can('channel_show')
                                            <a data-toggle="modal" data-target="#view-channel" class="edit edit-view" data-json="{{ $channel->with('user')->first() }}"><i class="fa fa-eye"></i></a>
                                        @endcan
                                        @can('channel_edit')
                                            <a class="edit" href="{{ route('admin.channels.edit', $channel->id) }}"><i class="fa fa-pen"></i></a>
                                        @endcan
                                        @can('channel_delete')
                                            <a class="delete" data-toggle="modal" data-id="{{ $channel->id }}"><i class="fa fa-trash-alt"></i></a>
                                        @endcan
                                    </td>
        ...
        <div class="modal fade modal-form" id="view-channel" tabindex="-1" role="dialog">
                            ...
                            <div class="card-body py-lg-2 mb-2">
                                ...
                                <div class="row">
                                    <div class="col-md-6 form-group">
                                        <label class="form-control-label">Channel ID</span></label>
                                        <input type="text" id="edit-view-channel-id" readonly class="form-control bg-white" value="LS_CH_02">
                                    </div>
                                    <div class="col-md-6 form-group">
                                        <label class="form-control-label">Channel Name</label>
                                        <input type="text" id="edit-view-name" class="form-control" placeholder="Channel Name" value="Senthil Balaji Finance">
                                    </div>
                                    <div class="col-md-6 form-group">
                                        <label class="form-control-label">Full Name</label>
                                        <input type="text" id="edit-view-admin-full-name" class="form-control" placeholder="Full Name" value="Senthil Balaji">
                                    </div>
                                    <div class="col-md-6 form-group">
    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.channels.create") }}">
                {{ trans('global.add') }} {{ trans('global.channel.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.channel.title_singular') }} {{ trans('global.list') }}
    </div>
            ...
            <table class=" table table-bordered table-striped table-hover datatable">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('global.channel.fields.name') }}
                        </th>
                        <th>
                            {{ trans('global.channel.fields.email') }}
                        </th>
                        <th>
                            {{ trans('global.channel.fields.email_verified_at') }}
                        </th>
                        <th>
                            {{ trans('global.channel.fields.roles') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($channels as $key => $channel)
                        <tr data-entry-id="{{ $channel->id }}">
                            ...
                            <td>
                                {{ $channel->email_verified_at ?? '' }}
                            </td>
                            <td>
                                @foreach($channel->roles as $key => $item)
...
<script>
    $(function () {
        $('.edit-view').click(function () {
            ...
            $('#edit-view-channel-id').val(data.channel_id);
            $('#edit-view-admin-full-name').val(data.admin_full_name);
        ...
        $('.delete').on('click', function (e) {
            ...
            var id = $(this).data('id');
            let url = "{{ route('admin.channels.destroy', '') }}";
            alert(url + '/' + id);
  ...
  let deleteButton = {
    ...
    url: " 'admin.channels.massDestroy' ",
